    ...
    public static void main(String[] args) {
        ...
        if (comecar.equalsIgnoreCase("sim")) {
            ...
            String[][] perguntas = {
                ...
                {"Qual a diferença entre == e .equals() em Java?", "A) Nenhuma, ambos comparam o conteúdo dos objetos", "B) == compara valores, .equals() compara tipos", "C) == compara referências de memória, .equals() compara o conteúdo", "D) == é usado apenas em números, .equals() apenas em strings", "B"},
                {"O que é herança em Java?", "A) O processo de salvar arquivos no computador", "B) Uma forma de compartilhar arquivos entre classes", "C) Um tipo especial de variável", "D) Quando uma classe herda atributos e métodos de outra classe", "C"},
                {"O que sao interface em java", "A) Classes que só podem ser usadas para entrada e saída de dados", "B) Classes abstratas obrigatórias em todo programa Java", "C) Contratos que definem métodos que uma classe deve implementar", "D) Progrmas de rede utilizadas em java" ,"D"},
                {"O que são exceções em Java?", "A) Erros que ocorrem durante a execução e podem ser tratados", "B) Tipos de variáveis com valor indefinido", "C) Funções especiais para repetição de código", "D) Comandos usados para encerrar o programa", "C"},
                {"O que é polimorfismo em Java?", "A) A capacidade de armazenar múltiplos arquivos em um só", "B) A possibilidade de executar múltiplos sistemas operacionais", "C) Quando objetos de diferentes classes se comportam como se fossem da mesma superclasse", "D) Um comando que encerra o programa abruptamente", "A"}
            };
